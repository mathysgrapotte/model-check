...
if (params.help) {
    ...
    log.info "--outdir                 The path to the directory where the files generated by the pipeline are saved."
    log.info "                         it can be a relative or absoluth path. Default './results/' ."
    log.info "                         Under such dir another one will be automaticly generated."
    log.info "                         It will be called workflow.runName + worflow.start = unique_run_name_and_time."
    log.info "                         In this subdir all the result from a specifc run will be placed. This is done,"
    log.info "                         to avoid overlap from differnt runs of the pipeline when using same input files and output dir."
    log.info "                         Since this pipeline trains many model in parallel any result file  specific to a given model"
    log.info "                         is saved in it's own specific subdirectory under '<outdir>/<unique_run_name_and_time>/' ."
